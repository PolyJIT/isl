...
{
	return "    public static class Ptr extends " + superPtrTy +
	       " { public Ptr() { super(); } }";
}
 ...
 */
string java_generator::paramtype2jna(QualType ty, bool wrapperTypes,
				     bool isBool)
{
	string type;
	if (is_isl_ctx(ty))
				    ...
				    const string &arg)
{
	...
	for (unsigned i = 0; i < n_arg - 1; ++i) {
		...
		os << paramtype2jna(arg_type) << " cb_arg" << dec << i << ", ";
	}
...
void java_generator::print_class(isl_class &clazz)
{
	...
	os << " {" << endl;
	string superPtr =
	    subclass ? (type2java(super) + ".Ptr") : "PointerType";
	...
	os << "    " << p_name << " as" << p_name << "() {" << endl
	   ...
	   << "            return this;" << endl << "        try {" << endl
	   << "            Constructor<" << p_name << "> c = " << p_name
	   << ".class.getConstructor(clazz);" << endl
	...
	impl_os << "    void " << name << "_free(" << p_name
		<< ".Ptr islobject);" << endl << "    " << p_name << ".Ptr "
	...
	for (in = clazz.methods.begin(); in != clazz.methods.end(); ++in) {
		...
		impl_os << "    " << rettype2jna(*in) << " " << fullname << "(";
...
void java_generator::generateClasses()
{
	...
	os_impl << commonHeader << "interface Impl extends Library {" << endl
		<< "    static Impl isl = (Impl)Native.loadLibrary(\"isl\", "
		   "Impl.class);" << endl << "    Ctx.Ptr isl_ctx_alloc();"
		<< endl << "    int isl_ctx_last_error(Ctx.Ptr ctx);" << endl
...

	{
		...
		os << commonHeader << "public class Ctx {" << endl
		   ...
		   << "    private Ctx() { ctx = Impl.isl.isl_ctx_alloc(); "
		   ...
		   << "        lastException = null;" << endl
		   << "        if (Impl.isl.isl_ctx_last_error(ctx) != 0) { // "
